package com.flitzen.adityarealestate.reminder;
...
public class AlarmSetter extends BroadcastReceiver {
  ...
  public void onReceive(Context context, Intent intent) {
    Utils.showLog("===AlarmSetter  onReceive ");
    ...
    try {
      while (cursor.moveToNext()) {
        ...
        if (ReminderType.fromString(type) == ReminderType.ALERT && time > Calendar.getInstance().getTimeInMillis()) {
          Intent service = new Intent(context, AlarmService.class);
          service.setAction(AlarmService.CREATE);
          service.putExtra(ReminderParams.ID, cursor.getInt(cursor.getColumnIndex(ReminderParams.ID)));
